"""
Prompts and Pydantic models for Grok AI analysis
"""
...
- Phone number: {phone}
- Email: {email}
- Office phone number: {office_number}
- Input location (city, state): {city}, {state}
...
class WebsiteAssessment(BaseModel):
    ...
    url: str = Field(description="Selected website URL or empty string if no match found")
    reason: str = Field(description="Brief explanation of why this website was selected")
...
class TeamPageSelection(BaseModel):
    """Team page selection result"""
    selectedUrl: str = Field(description="Single best URL for team/agents or empty string if none found")
    reasoning: str = Field(description="Concise 1-3 sentence explanation: which token(s) or rule matched")
...
class TeamSizeAnalysis(BaseModel):
    """Team size analysis result"""
    teamSize: float = Field(description="Total count of team members, or 0 if unable to determine")
    confidence: Literal["LOW", "MEDIUM", "HIGH"] = Field(description="Confidence level of the estimate")
    teamMembers: List[TeamMember] = Field(description="Array of individual team member details with name, email, phone, and designation/role")
    reasoning: str = Field(description="1 sentence explaining why the teamSize was chosen")
...
class TeamBrokerageExtraction(BaseModel):
    """Team and brokerage name extraction result"""
    team_name: Optional[str] = Field(None, description="The name of the real estate team, group, or associates (e.g., 'Leonard Corsi Team'). Returns null if not present.")
    brokerage_name: Optional[str] = Field(None, description="The name of the brokerage firm (e.g., 'Keller Williams', 'Compass'). Returns null if not present.")
